...
function searchDB(num)
{
	...
	if(num === "0")
	{	
		var x = $('.form-dbSearch').serialize();
		$("#searchDB1").val(document.getElementById("searchDB").value);		//keep search consistent			
		$("#searchModal").modal('show');
	}	
	else
	{
		console.log("hello!");
		var x = $('.form-dbSearch1').serialize();
		$("#searchDB").val(document.getElementById("searchDB1").value);		//keep search consistent
	}	
	console.log(x);
        $.ajax({
               type: "POST",
               url: "functions/searchdata.php",
               data: x,
	       dataType: "text",
               success: function (data) {
		console.log(data);
