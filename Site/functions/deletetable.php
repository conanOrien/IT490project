...
if (isset($_POST['browse'])) {
    ...
    if ($_POST['browse'] == "cr") {
        ...
        if ($result->num_rows > 0) {
           ...
           $data .='<tr><td>'.$row['Tail Number'].'</td><td>'.$row['Type'].'</td><td>'.$row['Fuel Count'].'</td><td><input type="checkbox" name="deleted[]" value="'.$row['Tail Number'].'"/></td></tr>';
            }
        ...
        if ($_POST['browse'] == "cre") {
            ...
            if ($result->num_rows > 0) {
                 while($row = $result->fetch_assoc()){
                $data .='<tr><td>'.$row['Crew ID'].'</td><td>'.$row['PilotName'].'</td><td>'.$row['NavigatorName'].'</td><td><input type="checkbox" name="deleted[]" value="'.$row['Crew ID'].'"/></td></tr>';
                 }
            }
        ...
        $data.='</tbody></table></form>'
                    . '<script>$("#deleteform").submit(function (event) {
                        event.preventDefault();
                         $.ajax({
                            type: "POST",
                            url: "functions/deleteaction.php",
