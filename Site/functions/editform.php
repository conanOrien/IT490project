...
if (isset($_POST['edited'])) {
    ...
    if ($_POST['type'] == "cr") {
       ...
       $sql = "SELECT * FROM aircraft_info where `Tail Number` =" . $id;
...
$row = $result->fetch_assoc();
        ...
        $data.='  <form id="editform">
            <input type="hidden" name="type" value="cr" /></th>
                ...
                <div class="form-group">  
                  ...
                  <input type="hidden" class="form-control" id="tailnumber" name="tailnumber" value="'.$row['Tail Number'] .'"  max="99999" required placeholder="12345"/>
                <div class="form-group">
                 ...
                 <div class="form-group">
                  ...
                  <input type="number" class="form-control" id="fuelcount" value="'.$row['Fuel Count'] .'"  name="fuelcount" max="99999" required placeholder="12345">
                </div>
       ...
       $sql = "SELECT * FROM aircrew where `Crew ID` =" . $id;
    $result = $conn->query($sql);
$row = $result->fetch_assoc();
            ...
            <input type="hidden" name="type" value="cre" /></th>
                ...
                <div class="form-group">  
                 <!-- <label for="crewid">Crew ID:</label>-->
                  <input type="hidden" class="form-control" id="crewid" value="'.$row['Crew ID'].'" name="crewid"  max="999" required placeholder="123"/>
                <div class="form-group">
               ...
               $sql = "SELECT * FROM aircraft_info ";
                ...
                if ($result->num_rows > 0) {
                   while($row = $result->fetch_assoc()){
                      if($row2['TailNum'] == $row['Tail Number']) {
                   $data .='<option value="'.$row['Tail Number'].'" selected>'.$row['Tail Number'].'</option>';
                      }else{
