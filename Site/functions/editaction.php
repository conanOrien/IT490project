...
if (isset($_POST['type'])) {
    ...
    if($_POST['type'] == "cr"){
        ...
        $sql = "Update aircraft_info set  Type ='$type1' , `Fuel Count` =$fuelcount  where `Tail Number` = $tailnumber ";
      ...
      if($_POST['type'] == "cre"){
        $crewid=trim($_POST['crewid']);
         ...
         $sql = "Update aircrew set  PilotName ='$pilotname' , NavigatorName ='$navigatorname'  where `Crew ID` = $crewid ";
         ...
         if($_POST['arrivaltime']=="null"){
            ...
            $departuretime = strtotime($departuretime);
             $arrivaltime = strtotime($arrivaltime);
                 ...
                 $sql = "Update flight set  TailNum =$tailnum , CrewID=$crewid
		 ,DepartureFrom='$departurefrom',DepartureTo='$departureto',DepartureTime=FROM_UNIXTIME($departuretime),ArrivalTime=FROM_UNIXTIME($arrivaltime),SkidNum=$skidnum  where FlightNum = $flightnum ";
